...
public class GridManager : MonoBehaviour
{
    ...
    public void ConnectTile(Vector2Int coordinates, Tile tile)
    {
        if (grid.ContainsKey(coordinates))
        {
            grid[coordinates].tile = tile;
        }
    }

    public Tile GetTileFromCords(Vector2Int coordinates)
    {
        if (grid.ContainsKey(coordinates)) { return grid[coordinates].tile; }
        else { return null; }
    }
